    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        function initView() {
          ...
          //louisro
          scope.tglIndeterminatePeriodShow = scope.inputObj.tglIndeterminatePeriodShow === true;
          scope.tglIndeterminatePeriod = scope.inputObj.tglIndeterminatePeriod ? scope.inputObj.tglIndeterminatePeriod : 'Inderterminate period';
          scope.indeterminatePeriod = {is: !!(scope.inputObj.isIndeterminatePeriod === true && scope.tglIndeterminatePeriodShow === true)};
          scope.tglIndeterminatePeriodClass = scope.inputObj.tglIndeterminatePeriodClass ? scope.inputObj.tglIndeterminatePeriodClass : 'toggle-energized';
          scope.titleIndeterminatePeriodClass = scope.inputObj.titleIndeterminatePeriodClass ? scope.inputObj.titleIndeterminatePeriodClass : 'energized energized-border';
...
*/
          ...
          scope.selectedDates.addRemove = function (year, month, date, state) {
            ...
            switch (scope.selectType) {
              ...
              default:
                if (state === undefined) {
                  if (find.isPresent) {
                    this.splice(find.i, 1); //remove
                  } else {
                    //louisro: prevent select day if 2 dates are already selected
                    if (scope.selectedDates.length < 2) {
                      this.push(new Date(year, month, date)); //add  
                    } else {
                      console.log('...two dates max');
                    }
                  }
                } else {
        ...
        //louisro: if indetermindate period is activated, only keep earliest date
        ...
        scope.onTap = function (date, row) {
          if (scope.accessType == ACCESS_TYPE.WRITE) {
            ...
            //louisro: activate/deactivate indeterminatePeriod button upon number of selected dates
